...
def clean_line_content(line):
    ...
    line = re.sub(PANDOC_LINK_PATTERN, r'\1', line)

    # 2. Eliminar anclajes sueltos <#i1i2>`__ 
    line = re.sub(PANDOC_ANCHOR_PATTERN, '', line)
...
def strip_metadata(content):
    ...
    content = re.sub(RST_HEADER_PATTERN, '', content)
    # Eliminar directivas contents antiguas 
    content = re.sub(CONTENTS_DIRECTIVE_PATTERN, '', content, flags=re.MULTILINE | re.IGNORECASE)
    return content